...
export async function POST() {
  try {
    ...
    try {
      ...
      // 检查响应状态
      if (!upstreamRes.ok) {
        console.error(`Upstream server returned error status: ${upstreamRes.status} ${upstreamRes.statusText}`)
      }

      const data = await upstreamRes.json().catch(() => {
        console.error('Upstream returned non-JSON data')
        return { success: false, message: '上游返回非JSON', timestamp: Date.now() }
      })
